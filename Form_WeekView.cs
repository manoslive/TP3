    ...
    public partial class Form_WeekView : Form
    {
        ...
        private void Fill_Days_Header(Graphics DC)
        {
            ...
            for (int dayNum = 0; dayNum < 7; dayNum++)
            {
                ...
                DC.DrawString(headerText, PN_DaysHeader.Font, brush, Convert.ToInt32(location.X +10), location.Y);//afficher à un pourcentage de la location
                DC.DrawString(headerDate, PN_DaysHeader.Font, brush, Convert.ToInt32(location.X +10), location.Y + 14);//""      
                date = date.AddDays(1);
            }
